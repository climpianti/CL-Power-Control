...
from .const import (
    ...
    CONF_WAIT_BETWEEN_SHEDS_SEC, CONF_WAIT_BEFORE_RESTORE_SEC, CONF_WAIT_BETWEEN_RESTORES_SEC,
    DEFAULT_LIMIT_W, DEFAULT_WARNING_W, DEFAULT_RESTORE_W,
    DEFAULT_DELAY_IMMEDIATE_SEC, DEFAULT_DELAY_WARNING_SEC,
    DEFAULT_WAIT_BETWEEN_SHEDS_SEC, DEFAULT_WAIT_BEFORE_RESTORE_SEC, DEFAULT_WAIT_BETWEEN_RESTORES_SEC,
    LOAD_ID, LOAD_NAME, LOAD_MIN_ACTIVE_W, LOAD_ESTIMATED_W, LOAD_ENERGY_TARGET_TEMP,
)
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for load in entry.data.get(CONF_LOADS, []):
        ...
        entities.extend([
            ...
            CLLoadNumber(coord, entry, load_id, LOAD_ENERGY_TARGET_TEMP, "Temperatura target energia", 0, 90, 0.5, "°C", "mdi:thermometer-plus"),
        ])
...
class _BaseNumber(CoordinatorEntity, NumberEntity):
    ...
    def __init__(self, coordinator, entry): super().__init__(coordinator); self._entry = entry; self._attr_device_info = _device(entry)
    @property
...
class CLGlobalNumber(_BaseNumber):
    def __init__(self, coordinator, entry, key, name, default, minimum, maximum, step, unit, icon):
        super().__init__(coordinator, entry); self._key = key; self._default = default; self._attr_name = name; self._attr_unique_id = f"{entry.entry_id}_installer_{key}"
        self._attr_native_min_value = minimum; self._attr_native_max_value = maximum; self._attr_native_step = step; self._attr_native_unit_of_measurement = unit; self._attr_icon = icon
    @property
    def native_value(self): return float(self.coordinator.conf(self._key, self._default))
    async def async_set_native_value(self, value):
        stored = int(value) if float(value).is_integer() else float(value); await self.coordinator.async_update_global_option(self._key, stored)


class CLLoadNumber(_BaseNumber):
    def __init__(self, coordinator, entry, load_id, field, label, minimum, maximum, step, unit, icon):
        super().__init__(coordinator, entry); self._load_id = load_id; self._field = field; self._label = label; self._attr_unique_id = f"{entry.entry_id}_load_{load_id}_{field}"
        self._attr_native_min_value = minimum; self._attr_native_max_value = maximum; self._attr_native_step = step; self._attr_native_unit_of_measurement = unit; self._attr_icon = icon
    def _load(self):
    ...
    @property
    def name(self):
        load = self._load(); return f"{load.get(LOAD_NAME, 'Carico')} {self._label}" if load else self._label
    @property
    def native_value(self):
        load = self._load(); return float(load.get(self._field, 0)) if load else 0.0
    async def async_set_native_value(self, value):
        stored = int(value) if float(value).is_integer() else float(value); await self.coordinator.async_update_load_field(self._load_id, self._field, stored)
