...
def _header_card(title: str, subtitle: str):
    return {"type": "markdown", "content": ('<table role="presentation" width="100%"><tr>' '<td width="104" valign="middle"><img src="/local/cl_power_control/header_logo.png" width="86"></td>' f'<td valign="middle"><span style="font-size:24px"><b>{title}</b></span><br>' f'<span style="font-size:14px">{subtitle}</span></td></tr></table>')}


def _load_card(hass, entry, load):
    load_id = load[LOAD_ID]; name = load.get(LOAD_NAME, "Carico"); rows = []
...
def _build(hass, entry):
    ...
    loads=entry.data.get(CONF_LOADS,[]); load_cards=[_load_card(hass,entry,load) for load in loads]; installer_load_cards=[_installer_load_card(hass,entry,load) for load in loads]; suspended_entities=[]
    for load in loads:
    ...
    power_cards.append({"type":"markdown","content":"Con gli avvisi attivi, quando il consumo supera una soglia CL Power Control mostra un avviso prima dell'intervento automatico, lasciando il tempo di ridurre i consumi. L'opzione è controllabile direttamente dal cliente."})
    ...
    for eid,name in [(energy_pv,"Fotovoltaico"),(energy_home,"Consumo casa"),(energy_grid,"Scambio rete"),(energy_surplus,"Surplus")]:
        if eid: primary_tiles.append({"type":"tile","entity":eid,"name":name})
    if primary_tiles: energy_cards.append({"type":"grid","columns":2,"square":False,"cards":primary_tiles})
    flow_rows=[]
    for eid,name in [(energy_import,"Prelievo rete"),(energy_export,"Immissione rete"),(energy_battery,"Potenza batteria"),(energy_soc,"SOC batteria"),(energy_charge,"Carica batteria"),(energy_discharge,"Scarica batteria")]:
        if eid: flow_rows.append({"entity":eid,"name":name})
    if flow_rows: energy_cards.append({"type":"entities","title":"Flussi energetici","show_header_toggle":False,"entities":flow_rows})
    graph_entities=[]
    for eid,name in [(energy_pv,"FV"),(energy_home,"Casa"),(energy_import,"Prelievo"),(energy_export,"Immissione")]:
        if eid: graph_entities.append({"entity":eid,"name":name})
    if graph_entities: energy_cards.append({"type":"history-graph","title":"Andamento energia","hours_to_show":6,"refresh_interval":15,"entities":graph_entities})
    ...
    if installer_mode:
        ...
        if locked_rows: installer_cards.append({"type":"conditional","conditions":[{"entity":installer_mode,"state":"off"}],"card":{"type":"entities","title":"🔒 Accesso installatore","show_header_toggle":False,"entities":locked_rows}})
        unlocked_rows=[]
...
async def async_create_dashboard(hass, entry) -> None:
    try:
        await async_install_assets(hass); async_register_frontend_resource(hass)
    except Exception as err:  # noqa: BLE001
        _LOGGER.exception("Unable to install CL Power Control dashboard assets: %s", err); return
