...
class CLPowerControlOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_load_add(self, user_input=None):
        if user_input is not None:
            ...
            return await self._save()
        defaults = {LOAD_PRIORITY: len(self._data.get(CONF_LOADS, [])) + 1}
    ...
    async def async_step_load_delete(self, user_input=None):
        ...
        choices = {item[LOAD_ID]: f"P{item.get(LOAD_PRIORITY, '?')} - {item.get(LOAD_NAME, 'Carico')}" for item in loads}
        if user_input is not None:
            selected = user_input["load_id"]
            self._data[CONF_LOADS] = sort_loads([x for x in loads if x.get(LOAD_ID) != selected])
            return await self._save()
    ...
    async def async_step_installer_unlock(self, user_input=None):
        ...
        return self.async_show_form(step_id="installer_unlock", data_schema=vol.Schema({
            vol.Required("pin"): TextSelector(TextSelectorConfig(type=TextSelectorType.PASSWORD))
        }), errors=errors)
