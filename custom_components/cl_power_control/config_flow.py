...
from homeassistant.helpers.selector import EntitySelector, EntitySelectorConfig, NumberSelector, NumberSelectorConfig, NumberSelectorMode, TextSelector, TextSelectorConfig, TextSelectorType, BooleanSelector
...
class CLPowerControlConfigFlow(config_entries.ConfigFlow,domain=DOMAIN):
    ...
    async def async_step_user(self,user_input:dict[str,Any]|None=None):
        ...
        if user_input is not None:
            pin=user_input.pop("installer_pin");confirm=user_input.pop("installer_pin_confirm")
            if len(pin)<4 or not pin.isdigit():errors["installer_pin"]="pin_format"
            elif pin!=confirm:errors["installer_pin_confirm"]="pin_mismatch"
            else:
                salt,digest=create_pin_hash(pin);data={CONF_NAME:user_input[CONF_NAME],CONF_INSTALLER_PIN_SALT:salt,CONF_INSTALLER_PIN_HASH:digest,CONF_LOADS:[]}
        ...
        schema=vol.Schema({vol.Required(CONF_NAME,default=NAME):TextSelector(TextSelectorConfig(type=TextSelectorType.TEXT)),vol.Required("installer_pin"):TextSelector(TextSelectorConfig(type=TextSelectorType.PASSWORD)),vol.Required("installer_pin_confirm"):TextSelector(TextSelectorConfig(type=TextSelectorType.PASSWORD))})
        return self.async_show_form(step_id="user",data_schema=schema,errors=errors)
    @staticmethod
    @callback
    def async_get_options_flow(config_entry):return CLPowerControlOptionsFlow(config_entry)


class CLPowerControlOptionsFlow(config_entries.OptionsFlow):
    def __init__(self,config_entry):self._data={**config_entry.data,**config_entry.options};self._selected_load_id=None;self._installer_unlocked=False
    def _shared_installer_unlocked(self):
        coordinator=self.hass.data.get(DOMAIN,{}).get(self.config_entry.entry_id);return bool(self._installer_unlocked or (coordinator and coordinator.installer_unlocked))
    def _mobile_services(self):
        services=self.hass.services.async_services().get("notify",{});return {f"notify.{name}":name.replace("mobile_app_","").replace("_"," ").title() for name in services if name.startswith("mobile_app_")}
    async def async_step_init(self,user_input=None):return self.async_show_menu(step_id="init",menu_options=["quick","notifications","loads","installer_unlock","installer_pin_reset"])
    ...
    async def async_step_loads(self,user_input=None):
        loads=sort_loads(list(self._data.get(CONF_LOADS,[])));options=["load_add"]
        if loads:options.extend(["load_select","load_delete"])
        options.append("init");return self.async_show_menu(step_id="loads",menu_options=options)
    async def async_step_load_add(self,user_input=None):
        if user_input is not None:
            loads=list(self._data.get(CONF_LOADS,[]));loads.append(new_load(user_input));self._data[CONF_LOADS]=sort_loads(loads);return await self._save()
    ...
    async def async_step_load_select(self,user_input=None):
        ...
        if user_input is not None:self._selected_load_id=user_input["load_id"];return await self.async_step_load_edit()
        choices={i[LOAD_ID]:f"P{i.get(LOAD_PRIORITY,'?')} - {i.get(LOAD_NAME,'Carico')}" for i in loads};return self.async_show_form(step_id="load_select",data_schema=vol.Schema({vol.Required("load_id"):vol.In(choices)}))
    ...
    async def async_step_load_delete(self,user_input=None):
        ...
        if user_input is not None:self._data[CONF_LOADS]=sort_loads([x for x in loads if x.get(LOAD_ID)!=user_input["load_id"]]);return await self._save()
    ...
    async def async_step_installer_pin_reset(self,user_input=None):
        ...
        if user_input is not None:
            ...
            else:salt,digest=create_pin_hash(pin);self._data[CONF_INSTALLER_PIN_SALT]=salt;self._data[CONF_INSTALLER_PIN_HASH]=digest;self._installer_unlocked=False;return await self._save()
    ...
    async def async_step_installer_pin_change(self,user_input=None):
        ...
        if user_input is not None:
            ...
            else:salt,digest=create_pin_hash(pin);self._data[CONF_INSTALLER_PIN_SALT]=salt;self._data[CONF_INSTALLER_PIN_HASH]=digest;return await self._save()
    ...
    async def _save(self):
        persistent_keys={CONF_NAME,CONF_INSTALLER_PIN_SALT,CONF_INSTALLER_PIN_HASH,CONF_LOADS};data={k:v for k,v in self._data.items() if k in persistent_keys};options={k:v for k,v in self._data.items() if k not in persistent_keys};self.hass.config_entries.async_update_entry(self.config_entry,data=data,options=options);return self.async_create_entry(title="",data=options)
