...
from homeassistant.helpers.selector import (
    ...
    NumberSelectorMode, TextSelector, TextSelectorConfig, TextSelectorType,
    BooleanSelector,
)
...
def _power_schema(defaults: dict) -> vol.Schema:
    return vol.Schema({
        vol.Optional(CONF_POWER_SENSOR, description={"suggested_value": defaults.get(CONF_POWER_SENSOR, "")}):
            EntitySelector(EntitySelectorConfig(domain="sensor", device_class="power")),
        vol.Required(CONF_LIMIT_W, default=defaults.get(CONF_LIMIT_W, DEFAULT_LIMIT_W)):
            NumberSelector(NumberSelectorConfig(min=100, max=100000, step=100, unit_of_measurement="W", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_WARNING_W, default=defaults.get(CONF_WARNING_W, DEFAULT_WARNING_W)):
            NumberSelector(NumberSelectorConfig(min=0, max=100000, step=100, unit_of_measurement="W", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_RESTORE_W, default=defaults.get(CONF_RESTORE_W, DEFAULT_RESTORE_W)):
            NumberSelector(NumberSelectorConfig(min=0, max=100000, step=100, unit_of_measurement="W", mode=NumberSelectorMode.BOX)),
    })


def _energy_schema(defaults: dict) -> vol.Schema:
    """Configure optional Energy Control sources and sign conventions."""
    return vol.Schema({
        vol.Required(CONF_ENERGY_ENABLED, default=defaults.get(CONF_ENERGY_ENABLED, DEFAULT_ENERGY_ENABLED)): BooleanSelector(),
        vol.Optional(CONF_GRID_POWER_SENSOR, description={"suggested_value": defaults.get(CONF_GRID_POWER_SENSOR, "")}):
            EntitySelector(EntitySelectorConfig(domain="sensor", device_class="power")),
        ...
        vol.Optional(CONF_PV_POWER_SENSOR, description={"suggested_value": defaults.get(CONF_PV_POWER_SENSOR, "")}):
            EntitySelector(EntitySelectorConfig(domain="sensor", device_class="power")),
        vol.Optional(CONF_BATTERY_POWER_SENSOR, description={"suggested_value": defaults.get(CONF_BATTERY_POWER_SENSOR, "")}):
            EntitySelector(EntitySelectorConfig(domain="sensor", device_class="power")),
        ...
        vol.Optional(CONF_BATTERY_SOC_SENSOR, description={"suggested_value": defaults.get(CONF_BATTERY_SOC_SENSOR, "")}):
...
def _timing_schema(defaults: dict) -> vol.Schema:
    return vol.Schema({
        vol.Required(CONF_DELAY_IMMEDIATE_SEC, default=defaults.get(CONF_DELAY_IMMEDIATE_SEC, DEFAULT_DELAY_IMMEDIATE_SEC)):
            NumberSelector(NumberSelectorConfig(min=1, max=300, step=1, unit_of_measurement="s", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_DELAY_WARNING_SEC, default=defaults.get(CONF_DELAY_WARNING_SEC, DEFAULT_DELAY_WARNING_SEC)):
            NumberSelector(NumberSelectorConfig(min=1, max=10800, step=1, unit_of_measurement="s", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_WAIT_BETWEEN_SHEDS_SEC, default=defaults.get(CONF_WAIT_BETWEEN_SHEDS_SEC, DEFAULT_WAIT_BETWEEN_SHEDS_SEC)):
            NumberSelector(NumberSelectorConfig(min=1, max=600, step=1, unit_of_measurement="s", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_WAIT_BEFORE_RESTORE_SEC, default=defaults.get(CONF_WAIT_BEFORE_RESTORE_SEC, DEFAULT_WAIT_BEFORE_RESTORE_SEC)):
            NumberSelector(NumberSelectorConfig(min=1, max=10800, step=1, unit_of_measurement="s", mode=NumberSelectorMode.BOX)),
        vol.Required(CONF_WAIT_BETWEEN_RESTORES_SEC, default=defaults.get(CONF_WAIT_BETWEEN_RESTORES_SEC, DEFAULT_WAIT_BETWEEN_RESTORES_SEC)):
            NumberSelector(NumberSelectorConfig(min=1, max=3600, step=1, unit_of_measurement="s", mode=NumberSelectorMode.BOX)),
    })
...
def _load_schema(defaults: dict | None = None) -> vol.Schema:
    ...
    return vol.Schema({
        ...
        vol.Required(LOAD_MIN_ACTIVE_W, default=defaults.get(LOAD_MIN_ACTIVE_W, 10)):
            NumberSelector(NumberSelectorConfig(min=0, max=5000, step=1, unit_of_measurement="W", mode=NumberSelectorMode.BOX)),
        vol.Required(LOAD_ESTIMATED_W, default=defaults.get(LOAD_ESTIMATED_W, 0)):
            NumberSelector(NumberSelectorConfig(min=0, max=100000, step=10, unit_of_measurement="W", mode=NumberSelectorMode.BOX)),
    })
...
class CLPowerControlOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_load_add(self, user_input=None):
        if user_input is not None:
            ...
            return await self._save()
        return self.async_show_form(step_id="load_add", data_schema=_load_schema({LOAD_PRIORITY: len(self._data.get(CONF_LOADS, [])) + 1}))

    async def async_step_load_select(self, user_input=None):
        loads = sort_loads(list(self._data.get(CONF_LOADS, [])))
        if user_input is not None:
            self._selected_load_id = user_input["load_id"]; return await self.async_step_load_edit()
        choices = {item[LOAD_ID]: f"P{item.get(LOAD_PRIORITY, '?')} - {item.get(LOAD_NAME, 'Carico')}" for item in loads}
    ...
    async def async_step_load_edit(self, user_input=None):
        ...
        if user_input is not None:
            updated = {**current, **user_input, LOAD_ID: current[LOAD_ID]}; self._data[CONF_LOADS] = sort_loads([updated if x.get(LOAD_ID) == current[LOAD_ID] else x for x in loads]); return await self._save()
    ...
    async def async_step_load_delete(self, user_input=None):
        loads = sort_loads(list(self._data.get(CONF_LOADS, []))); choices = {item[LOAD_ID]: f"P{item.get(LOAD_PRIORITY, '?')} - {item.get(LOAD_NAME, 'Carico')}" for item in loads}
        if user_input is not None:
            selected = user_input["load_id"]; self._data[CONF_LOADS] = sort_loads([x for x in loads if x.get(LOAD_ID) != selected]); return await self._save()
    ...
    async def async_step_installer_pin_reset(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if len(pin) < 4 or not pin.isdigit(): errors["new_pin"] = "pin_format"
                ...
                salt, digest = create_pin_hash(pin); self._data[CONF_INSTALLER_PIN_SALT] = salt; self._data[CONF_INSTALLER_PIN_HASH] = digest; self._installer_unlocked = False; return await self._save()
    ...
    async def async_step_installer_pin_change(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if len(pin) < 4 or not pin.isdigit(): errors["new_pin"] = "pin_format"
                ...
                salt, digest = create_pin_hash(pin); self._data[CONF_INSTALLER_PIN_SALT] = salt; self._data[CONF_INSTALLER_PIN_HASH] = digest; return await self._save()
